...
We zullen zowel berekeningen doen o.b.v. de kaarten van 2022 en 2023.
...
```{r, cache=TRUE}
parcels_grouped_2022 <- calc_parcel_size(lbg_layer_2022) %>%
  mutate(jaar = 2022)
parcels_grouped_2023 <- calc_parcel_size(lbg_layer_2023) %>%
  mutate(jaar = 2023)
```
...
```{r}
bind_rows(parcels_grouped_2022, parcels_grouped_2023) %>%
  st_drop_geometry() %>%
  group_by(jaar) %>%
  summarise(
    min = min(area_ha),
