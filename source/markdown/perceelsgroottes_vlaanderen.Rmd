...
for (spec in specs) {
  for (year in 2022:2023) {
    ...
    select_ids <- intersect_sbp_sf %>%
      st_drop_geometry() %>%
      pull(ID)
    sbp_parcels <- overlay_sf %>%
      st_drop_geometry() %>%
      filter(ID %in% select_ids) %>%
      mutate(soort = spec)
    
    out_list_sbp[[i]] <- sbp_parcels
  }
}
sbp_parcel_df <- do.call(rbind.data.frame, out_list_sbp)
```
...
file_intersect_akker <- file.path(cache_path, "intersect_sbp_akker.rds")
if (file.exists(file_intersect_akker)) {
  intersect_sbp_akker <- readRDS(file_intersect_akker)
} else {
  intersect_sbp_akker <- st_intersection(sbp_akker_union, parcels_grouped_sf)
  saveRDS(intersect_sbp_akker, file_intersect_akker)
}

intersect_sbp_akker <- qgis_run_algorithm(
  algorithm = "native:intersection",
  INPUT = sbp_akker_union,
  OVERLAY = parcels_grouped_sf)

# Select parcels that overlap SBP
select_ids <- intersect_sbp_akker %>%
  st_drop_geometry() %>%
  pull(ID)
sbp_akker_parcels <- parcels_grouped_sf %>%
  filter(ID %in% select_ids)
```
...
```{r}
sbp_akker_parcels_summary <- sbp_akker_parcels %>%
  group_by(jaar) %>%
  summarise(
    min = min(area_ha),
    Q10 = quantile(area_ha, 0.1),
